...
- 知识库存放在一个根目录下，包含多种文件类型（如 `.md`/`.txt`、`.pdf`、`.xlsx`、`.json` 等），通常按类型或业务用途拆分为多级子目录。
- 采用**分层目录索引文件**：
...
1. **首先：确认 JSON 结构**
   - 在处理任何 JSON 之前，先确认其顶层结构是数组还是对象
   - 快速探查结构，不要一开始整文件读取：
     - 优先查看前 1-5 条记录
     - 确认核心字段名，如 `question`、`answer`、`label`、`url`
     - 确认是否存在嵌套字段、可选字段、空值字段
   - 默认每条记录通常包含标准问答字段
   - 如字段名与预期不一致，先记录实际 schema，再决定后续检索方式
   - **禁止**一开始就把整个 JSON 文件全量读入上下文

2. **选择候选 JSON**
   - 根据 `data_structure.md`、文件名、路径和业务域判断相关度
   - 若用户明确指定某个 JSON 文件，则优先使用该文件
   - 若存在多个同类 JSON，优先选择：
     - 与用户问题领域最接近的分类目录下的文件

3. **应用结构化检索，而不是纯文本整扫**
   - 优先把 JSON 当作结构化问答数据处理，而不是普通文本文件
   - 检索顺序建议如下：
     1. 先检索 `label` / 分类字段
     2. 再检索 `question`
   - 对用户问题提取 3-8 个关键词，并补充：
     - 同义词
     - 中英文变体
     - 常见缩写
     - 口语化问法
   - 对每轮命中结果，只保留少量候选记录（如 top 5-10），不要一次把全部命中项送入上下文

4. **局部读取与字段级返回**
   - 对命中的 JSON 记录，只读取和返回必要字段：
     - `question`
     - `answer`
     - `label`
     - `url`
     - 必要时附加记录索引或唯一 ID
   - 如 `answer` 很长，只截取与问题最相关的片段，不要整段无差别展开
   - 输出时保留“命中的哪个字段”信息，例如：
     - 命中 `label`
     - 命中 `question`
     - 命中 `answer`
...
5. **特殊处理**
   - 如果 `question` 命中较弱，但 `answer` 中存在强相关术语，可保留该条作为补充证据，而不是主命中
   - 如果多条记录内容高度重复：
     - 优先保留表述最完整、分类最准确、的那条
     - 其余条目作为候选补充，不全部展开
   - 如果 JSON 存在编码异常、乱码、特殊符号污染：
     - 先做最小必要的清洗与规范化
     - 再进行关键词匹配
   - 如果用户问题明显属于“意图匹配”而不是“关键词原词匹配”，应进行多轮改写检索，而不是只搜一次原词

6. **多轮迭代检索**
   - 应用「多轮迭代检索机制」（见上文公共检索原则）
   - 每轮检索后都要判断：
     - 是否已找到直接回答用户问题的记录
     - 是否只是找到相关分类，还需要继续缩小范围
     - 是否需要更换关键词、同义词或换到 `answer` 字段继续检索
   - 最多进行 5 轮，避免无效反复扫描
...
7. **结果组织方式**
   ...
   - 最终回答时：
     - 先给结论
     - 再列出最相关的 1-3 条记录作为依据
     - 若没有完全匹配项，明确说明“未找到直接命中”，并给出最接近的记录

### JSON 检索注意事项

- 禁止一开始整文件读入上下文
- 禁止只靠 `answer` 大段模糊命中就直接下结论
- 优先做字段级过滤，再做局部展开
- 如 JSON 本质上是“标准问答对”，应优先按“问题匹配”思路检索，而不是按全文搜索思路处理
- 若用户问题与分类强相关但表述不同，应先做问题改写，再检索
...
- 回答风格
  - 尽量用用户提问的语言（中文/英文）作答。
  - 先给出结论，再给出简要依据。
  - 如需要，可在后面列出引用的文件和大致位置，以及具体的文本，例如：
    - 来源：design/api_gateway.md 第 100 行附近，具体引用内容是.............
    - 来源：reports/2023_Q1_sales.xlsx Summary 工作表，具体引用内容是.............
- 信息缺失或不确定时
