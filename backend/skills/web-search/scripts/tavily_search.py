...
def first_env_value(env_file_values: dict[str, str], *names: str) -> str:
    for name in names:
...
def main() -> int:
    ...
    api_key = first_env_value(env_file_values, "TAVILY_API_KEY")
    if not api_key:
        return emit_json(
            {
                "ok": False,
    ...
    project_id = args.project_id or first_env_value(env_file_values, "TAVILY_PROJECT")
    headers = build_headers(api_key, project_id or None)
